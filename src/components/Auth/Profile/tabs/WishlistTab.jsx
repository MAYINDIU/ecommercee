...
export default function WishlistTab({ className }) {
  ...
  useEffect(() => {
    fetch(
      `https://habib.munihaelectronics.com/public/api/SingleWishlist/${userdata?.id}`
    )
      .then((res) => res.json())
          ...
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <tbody>
              ...
              <tr className="text-[13px] font-medium text-black bg-[#F6F6F6] whitespace-nowrap px-2 border-b default-border-bottom uppercase">
                <td className="py-4 whitespace-nowrap text-center">SL No</td>
              ...
              {list?.map((l, i) => (
                <tr className="bg-white border-b hover:bg-gray-50">
                  <td className="pl-10  py-4">
                    <span className="text-[15px] text-center font-normal">
                      {i + 1}
                    </span>
                  </td>
                  <td className="pl-10  py-4 ">
